...
ui <- page_navbar(
  title = tags$span(
    tags$img(src = "https://www.r-project.org/logo/Rlogo.png", height = "24px", style = "margin-right:8px; vertical-align:middle;"),
  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        textInput("base_dir", "Base directory path:", value = "/input", placeholder = "/path/to/data"),

        hr(),
        h5("🧬 Genome Parameters", class = "mt-2 mb-1 fw-bold"),
        numericInput("genome_length", "Genome length (bp):", value = 238080, min = 1000),
        numericInput("bin_size", "Bin size (bp):", value = 1000, min = 100, max = 10000, step = 100),
        numericInput("oriLyt_pos", "OriLyt position (bp):", value = 150000, min = 0),
...
2_WTnoGCV_1_S16,WT-GCV
3_WTnoGCV_2_S17,WT-GCV
4_WTnoGCV_3_S18,WT-GCV
5_WTnoGCV_4_S19,WT-GCV
6_WTnoGCV_5_S20,WT-GCV
7_WTyesGCV_1_S21,WT+GCV
8_WTyesGCV_2_S22,WT+GCV
9_WTyesGCV_3_S23,WT+GCV"
        ),
      ...
      card(
        card_header("ℹ️ File Path Preview"),
  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        sliderInput("dist_xlim", "X-axis limit (bp from OriLyt):",
                    min = 5000, max = 120000, value = 50000, step = 1000),

        numericInput("loess_span", "LOESS span:", value = 0.3, min = 0.05, max = 1.0, step = 0.05),
  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        selectInput("stat_test", "Test to run:",
                    choices = c(
                      "One-way ANOVA" = "anova",
  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        numericInput("sample_xlim", "X-axis max (bp):", value = 50000, min = 5000)
      ),
      card(
        full_screen = TRUE,
...
server <- function(input, output, session) {
  ...
  sample_info <- reactive({
    ...
    tryCatch(
      read.csv(text = input$sample_metadata_text, stringsAsFactors = FALSE),
  ...
  output$filepath_preview <- renderText({
    ...
    paste0(
      "Example file path:\n",
      file.path(input$base_dir, example_sample, "08_mutation_identification", "Exported.coverage.tab")
    )
  })
  ...
  observeEvent(input$load_data, {
    ...
    withProgress(message = "Loading samples...", value = 0, {
      ...
      for (i in seq_len(n)) {
        ...
        file_path <- file.path(base_dir, sname, "08_mutation_identification", "Exported.coverage.tab")
      ...
      if (length(errors) > 0) {
        load_log(paste0("⚠️ Loaded with errors:\n", paste(errors, collapse = "\n")))
      } else {
  ...
  output$load_status <- renderUI({
    msg <- load_log()
    if (msg == "") return(NULL)
    cls <- if (grepl("✅", msg)) "alert alert-success" else if (grepl("⚠️", msg)) "alert alert-warning" else "alert alert-info"
  ...
  coverage_plot_obj <- reactive({
    ...
    if (input$use_region_filter) {
      dat <- dat[bin_mid >= input$region_start & bin_mid <= input$region_end]
    }
    ...
    p <- p +
      geom_smooth(method = "loess", span = input$loess_span, se = TRUE, linewidth = 1.2) +
  ...
  output$download_cov_plot <- downloadHandler(
    ...
    content  = function(file) ggsave(file, coverage_plot_obj(), width = 10, height = 6, dpi = 300)
  )

  output$download_cov_table <- downloadHandler(
    filename = "all_samples_binned_norm_cov.csv",
    content  = function(file) fwrite(all_data(), file)
  )
  ...
  slopes_plot_obj <- reactive({
    ...
    p <- ggplot(sl, aes(x = group, y = slope, fill = group)) +
      geom_boxplot(
        alpha = 0.65,
        outlier.shape = if (input$show_outlier) 19 else NA
      ) +
      scale_fill_brewer(palette = "Set1") +
      labs(
        title = "Distribution of Coverage Decay Slopes",
        x = "Group", y = "Slope (log-linear decay)"
      ) +
      theme_minimal(base_size = 14) +
      theme(legend.position = "none", plot.title = element_text(face = "bold"))

    if (input$show_jitter) {
      p <- p + geom_jitter(width = 0.2, size = 2.5, alpha = 0.8)
    }
    p
  })

  output$slopes_plot <- renderPlot({ slopes_plot_obj() })
  ...
  output$stat_test_output <- renderPrint({
    ...
    isolate({
      if (input$stat_test == "anova") {
        ...
        res <- pairwisePermutationTest(slope ~ group, data = as.data.frame(sl), distribution = "approximate")
        print(res)
      }
    })
  })
  ...
  output$download_slope_plot <- downloadHandler(
    ...
    content  = function(file) ggsave(file, slopes_plot_obj(), width = 6, height = 8, dpi = 300)
  )

  output$download_slope_table <- downloadHandler(
    filename = "coverage_decay_slopes.csv",
    content  = function(file) fwrite(slopes_data(), file)
  )
  ...
  output$sample_plot <- renderPlot({
    ...
    y_var <- if (input$log_scale_sample) "log_norm_cov" else "norm_cov"
    y_lab <- if (input$log_scale_sample) "Log Normalized Coverage" else "Normalized Coverage"

    ggplot(sdat, aes_string(x = "dist_from_ori", y = y_var)) +
      geom_line(color = "#2C7BB6", linewidth = 0.8) +
      geom_smooth(method = "lm", color = "#D7191C", linetype = "dashed", se = FALSE) +
